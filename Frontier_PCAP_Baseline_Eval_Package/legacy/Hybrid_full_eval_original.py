...
DOCX_QA_PATH = "Evaluation_results/Backdoor_test2.docx"
...
def rerank_docs(query, docs, reranker, top_k):
    ...
    return [doc for _, doc in sorted(zip(scores, docs), reverse=True)][:top_k]
...
def main():
    ...
    If the retrieved chunks do not explicitly contain the full answer, respond with the best possible answer based solely on the available content. 
    ...
    Question: {question}
        ...
        scores = evaluate_answer(response, answer)
        resp_size, token_count = calculate_response_size(response)
        ...
        result_rows.append({"question": question, "reference": answer, "rag_answer": response, **{f"rag_{k}": v for k,v in scores.items()}})
        metric_rows.append({
            "question": question,
            "elapsed_time": t1 - t0,
    ...
    df_results.to_csv("Evaluation_results/eval_hybrid_full_llama3.1:8b.csv", index=False)

    df_metrics = pd.DataFrame(metric_rows)
    df_metrics.to_csv("Evaluation_results/system_metrics_hybrid_full_llama3.1:8b.csv", index=False)
    ...
    pd.DataFrame([combined_averages]).to_csv("Evaluation_results/combined_averages_hybrid_full_llama3.1:8b.csv", index=False)
